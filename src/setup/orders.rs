...
fn snapshot_items(
    ...
    for i in 0..order_model.row_count() {
        ...
        let (menu_item_id, name, code, category, cost_price) = match meta {
            ...
            None => (None, line.name.to_string(), String::new(), String::new(), 0.0),
        };
        ...
        let _ = db::orders::add_order_item_snapshot(
            conn, order_id, menu_item_id, &name, &code, &category,
            unit_price, cost_price, line.quantity, line_total, &line.note,
        );
    }
}
...
fn save_bill_to_order(
    ...
    let _ = db::orders::update_order_totals(
        conn, order_id, subtotal, cgst, sgst, gst,
...
fn save_payments(conn: &rusqlite::Connection, order_id: i64, window: &MainWindow) {
    let _ = db::payments::clear_payments(conn, order_id);
    let mode = window.get_payment_mode();
    let total = window.get_bill_total() as f64;

    match mode {
        1 => { let _ = db::payments::add_payment(conn, order_id, "cash", total, None); }
        2 => { let _ = db::payments::add_payment(conn, order_id, "upi", total, None); }
        3 => { let _ = db::payments::add_payment(conn, order_id, "card", total, None); }
        4 => {
            ...
            if cash > 0.0 { let _ = db::payments::add_payment(conn, order_id, "cash", cash, None); }
            if upi > 0.0 { let _ = db::payments::add_payment(conn, order_id, "upi", upi, None); }
            if card > 0.0 { let _ = db::payments::add_payment(conn, order_id, "card", card, None); }
        }
        5 => { let _ = db::payments::add_payment(conn, order_id, "due", 0.0, None); }
        _ => {}
    }
}

fn payment_status_for_mode(mode: i32) -> &'static str {
    match mode { 1 | 2 | 3 | 4 => "paid", 5 => "unpaid", _ => "unpaid" }
}
...
pub fn setup_order_persistence(
    ...
    let open_orders: Rc<RefCell<HashMap<String, i64>>> = Rc::new(RefCell::new(HashMap::new()));
    ...
    window.on_open_table(move |table_name: slint::SharedString| {
        ...
        let (Some(window), Some(order_model)) = (
            weak_window_open.upgrade(), weak_order_lines_open.upgrade(),
        ) else { return; };
        ...
        let order_id = if let Some(id) = open_orders_open.borrow().get(&table) {
            ...
            db::orders::get_open_order_by_table(&conn_open, &table)
                .ok().flatten().map(|o| o.id)
        };

        match order_id {
            Some(id) => {
                if let Ok(items) = db::orders::get_order_items(&conn_open, id) {
                    let lines: Vec<OrderLine> = items.into_iter().map(|item| OrderLine {
                        name: item.name.into(),
                ...
                if let Ok(Some(order)) = db::orders::get_open_order_by_table(&conn_open, &table) {
                    ...
                    window.set_packaging_text(format!("{:.2}", order.packaging).into());
                    window.set_delivery_text(format!("{:.2}", order.delivery).into());
                    window.set_discount_text(format!("{:.2}", order.discount).into());
    ...
    window.on_save_order(move || {
        let (Some(window), Some(order_model)) = (
            weak_window_save.upgrade(), weak_order_lines_save.upgrade(),
        ) else { return; };

        let table = window.get_current_bill_table().to_string();
        let order_id = get_or_create_order_id(&conn_save, &open_orders_save, &table);
        save_bill_to_order(&conn_save, order_id, &order_model, &menu_items_save, &window);
    ...
    window.on_save_and_close_order(move || {
        let (Some(window), Some(order_model)) = (
            weak_window_save_close.upgrade(), weak_order_lines_save_close.upgrade(),
        ) else { return; };

        let table = window.get_current_bill_table().to_string();
        let order_id = get_or_create_order_id(&conn_save_close, &open_orders_save_close, &table);
        save_bill_to_order(&conn_save_close, order_id, &order_model, &menu_items_close, &window);

        let bill_no = db::sequences::next_bill_number(&conn_save_close).unwrap_or_default();
        let area_name = table.split(' ').next().unwrap_or("").to_string();
        let payment_status = payment_status_for_mode(window.get_payment_mode());

        let _ = db::orders::close_order(
            &conn_save_close, order_id, &bill_no, &area_name, payment_status,
        );
    ...
    window.on_close_bill(move || {
        let (Some(window), Some(order_model)) = (
            weak_window_back.upgrade(), weak_order_lines_back.upgrade(),
        ) else { return; };
        ...
        if order_model.row_count() == 0
            && subtotal == 0.0 && packaging == 0.0 && delivery == 0.0 && discount == 0.0
        {
            order_model.set_vec(Vec::<OrderLine>::new());
            window.set_current_view(0);
            return;
        ...
        let table = window.get_current_bill_table().to_string();
        let order_id = get_or_create_order_id(&conn_back, &open_orders_back, &table);
        save_bill_to_order(&conn_back, order_id, &order_model, &menu_items_back, &window);
    ...
    window.on_move_table(move |new_table_name: slint::SharedString| {
        let Some(window) = weak_window_move.upgrade() else { return; };

        let new_table = new_table_name.to_string();
        let old_table = window.get_current_bill_table().to_string();
        if old_table == new_table || new_table.is_empty() { return; }

        let old_order_id = open_orders_move.borrow().get(&old_table).copied()
            .or_else(|| {
                db::orders::get_open_order_by_table(&conn_move, &old_table)
                    .ok().flatten().map(|o| o.id)
            });

        let new_order_id = open_orders_move.borrow().get(&new_table).copied()
            .or_else(|| {
                db::orders::get_open_order_by_table(&conn_move, &new_table)
                    .ok().flatten().map(|o| o.id)
            });

        if new_order_id.is_some() { return; }

        if let Some(order_id) = old_order_id {
            let _ = db::orders::update_order_table(&conn_move, order_id, &new_table);
            open_orders_move.borrow_mut().remove(&old_table);
            open_orders_move.borrow_mut().insert(new_table.clone(), order_id);
        }
