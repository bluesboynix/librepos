...
Built with **Rust** and **Slint** (a native GUI toolkit). All data is stored locally in
SQLite — no internet, no cloud, no subscription.
...
Categories are derived automatically — type any new category name on an item
and it appears as a tab in the Menu view.

Daily Workflow
Taking an order
Dashboard — click a table card

The Bill view opens with three panels:
...
Click a menu item (or its + button) to add it to the order

Adjust quantity with −/+, or remove with X

Add item notes with the N button
...
Sidebar → Reports → Bills tab

Search by bill number, table, area, customer name, or phone

Click a row to open the bill detail
...
End of day
Reports → Summary → set range to Today

Check total sales, order count, payment breakdown

Reports → Bills → Export CSV for accounting

Back up librepos.db (see Data & Backup)
...
Click Change next to an action
...
Note: the in-app preview may clip very long item lists. The PDF export always
contains every item.
...
Stored in the areas table with a sort_order column that determines the
...
The rate is applied to every new bill view. Existing closed bills keep their
snapshot values.
...
This is an alpha release intended for testing in real restaurant environments.
Please be aware of the following:

Bill preview scroll — long item lists may be clipped in the on-screen
preview. Use the PDF export for a complete receipt.

No printer integration — receipts are exported as PDF files. You'll need
to print them via your operating system's print dialog, or send them to
customers via WhatsApp / email.

Single user only — no login, no roles, no access restrictions.

No cloud sync — each machine has its own database. To share data across
machines, copy librepos.db manually.

No refund workflow — voided bills are excluded from reports, but there's
no separate "refund" concept. Use Void for cancellations.

No stock tracking — ingredients are used for cost calculation only.
Inventory levels are not decremented on sales. Track stock externally.

No table reservations or merging — tables can be moved but not merged.

Reports are read-only — no way to edit past bills except through the
Edit action on individual bills.
...
Version — the small label at the top right of the app (e.g. v0.4.0-alpha)

Environment — OS, screen resolution, whether it happens every time

Logs / screenshots — terminal output, screenshots of the UI

Sample data — if the issue involves specific menu items or bills, describe them
...
Suggested options for an open-source POS:

MIT — permissive, best for wide adoption

GPLv3 — copyleft, ensures derivatives stay open

Apache 2.0 — permissive, includes patent grant

Until a licence is added, the code is "all rights reserved" by default, which
means others cannot legally reuse it. If you intend to release this publicly,
add a LICENSE file with the appropriate text.
...
Author
Your name / team / business

Thank you for testing LibrePOS. Feedback is welcome and appreciated.

text

### Notes on what to edit

- Replace `<your-repo-url>` and the "Reporting Issues" section with your real GitHub or GitLab URL.
- Add the LICENSE section header with the actual licence you choose (create a `LICENSE` file too).
- Fill in the Author section.
- If you want a starter DB or sample data included with the alpha, mention it in the Installation section.

### Then tag the release

```bash
git add README.md
git commit -m "Add README for v0.4.0-alpha"
